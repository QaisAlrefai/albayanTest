...
class MenuBar(QMenuBar):
    ...
    # Create actions using a loop
        ...
        self.change_repeat_limit_action = QAction("تغيير عدد مرات تكرار الآية", self)
        self.change_repeat_limit_action.triggered.connect(self.open_repeat_limit_settings)

        self.change_text_repeat_action = QAction("تغيير عدد مرات تكرار النص", self)
        self.change_text_repeat_action.triggered.connect(self.open_text_repeat_settings)

        self.change_after_text_action = QAction("تغيير الإجراء بعد النص", self)
        self.change_after_text_action.triggered.connect(self.open_after_text_settings)
        ...
        self.preferences_menu.addAction(self.change_text_repeat_action)
        self.preferences_menu.addAction(self.change_after_text_action)
    ...
    def open_text_repeat_settings(self):
        dialog = SettingsDialog(self.parent)
        dialog.open_listening_tab_and_focus_text_repeat()
        dialog.exec()


    def open_after_text_settings(self):
        dialog = SettingsDialog(self.parent)
        dialog.open_listening_tab_and_focus_action_after_text()
        dialog.exec()
