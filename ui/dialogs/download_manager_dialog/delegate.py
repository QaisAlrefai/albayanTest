...
class DownloadDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter: QPainter, option: QStyleOptionViewItem, index):
        ...
        if progress_data:
            ...
            def fmt(b):
                return f"{b / (1024*1024):.2f} MB" if b > 1024*1024 else f"{b / 1024:.2f} KB"
        ...
        title_rect = QRect(content_rect.left(), content_rect.top(), content_rect.width(), title_height)
        
        painter.drawText(title_rect, Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, filename)
        ...
        status_rect = QRect(content_rect.left(), content_rect.top(), content_rect.width(), title_height)
        ...
        size_rect = QRect(content_rect.left(), title_rect.bottom() + 5, content_rect.width(), title_height)
        painter.drawText(size_rect, Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter, size_text)

        # 3. Progress Bar (Bottom)
        bar_rect = QRect(content_rect.left(), content_rect.bottom() - self.bar_height, content_rect.width(), self.bar_height)
        
        # Background bar
        bg_bar_color = QColor(220, 220, 220)
        if option.state & QStyle.StateFlag.State_Selected:
             bg_bar_color = QColor(255, 255, 255, 100) # Semi-transparent white on selection
        ...
        if percentage > 0 or status == DownloadStatus.DOWNLOADING:
            ...
            progress_rect = QRect(bar_rect.left(), bar_rect.top(), progress_width, bar_rect.height())
